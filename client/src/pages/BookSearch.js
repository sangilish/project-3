...
const BookSearch = () => {
  ...
  const calculateScanCost = (price) => {
    if (!price) return 0;
    return (price * 0.05).toFixed(2); // $100당 $5
  };
          ...
          <CardContent>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <Typography variant="body1">
                  Google Books Price: ${selectedBook.saleInfo?.listPrice?.amount || 'N/A'}
                </Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                {amazonLoading ? (
                  ...
                  <Typography variant="body1">
                    Amazon Price: ${amazonPrice?.SearchResult?.Items?.[0]?.Offers?.Listings?.[0]?.Price?.Amount || 'N/A'}
                  </Typography>
                ) : (
                  <Typography variant="body1">
            ...
            <Box sx={{ mt: 2 }}>
              <Typography variant="body1">
                Estimated Scan Cost: ${calculateScanCost(selectedBook.saleInfo?.listPrice?.amount)}
              </Typography>
            </Box>
